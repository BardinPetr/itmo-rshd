...
$$
    ...
    BEGIN
        ...
        -- fetch table oid
        SELECT cls.oid, stat.n_live_tup
        INTO target_table_oid, row_cnt
        FROM pg_class cls
        ...
        FOR col IN
            ...
            FROM pg_attribute att
                     ...
                     LEFT JOIN pg_stats stat
                               ON stat.schemaname = target_table_schema
                                   AND stat.tablename = target_table_name
                                   AND stat.attname = att.attname
            WHERE att.attnum > 0
              AND att.
                      attrelid = target_table_oid
              AND NOT att.attisdropped
            ORDER BY ord
            LOOP
                tmp_distinct := CASE WHEN col.c_distinct < 0 THEN col.c_distinct * row_cnt ELSE col.c_distinct END;
